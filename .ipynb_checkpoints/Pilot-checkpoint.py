...
try :
    _p=PWM(1,0x5e) #AutoCar and Prime
...
class CameraPod:
    ...
    def tiltBack(self):
        self.pwm.setDuty(self.Horizontal_GPIO,self._angle2duty(180+self.tiltCenterAngle))
...
class AutoCar(axis6):
    ...
    def steering(self, value):
        ...
        if value < 0:
            self.wheel.turnLeft(int(abs(value) * self.steer_limit * 1.2))
        else:
...
class Object_Follow(_cam_based_class):
    ...
    def load_model(self,path='ssd_mobilenet_v2_coco.engine'):
        try:
    ...
    def detect(self, image=None, index=None, show=True, callback=None):
        if image is None:
